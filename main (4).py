...
def inch_swap_vst(privatekey):
    try:
        ...
        def get_api_call_data(url):
            try: 
                call_data = requests.get(url, proxies = proxy_servers, auth = auth[i])
...
def change_proxy():
    try:
        ...
        print('Не могу сменить прокси, ошибка:', str(e))
...
with open('proxy.txt', 'r') as file:
    ...
    for item in proxys:
        ...
        http_proxy.append('http://' + proxy)
        auth.append(HTTPProxyDigestAuth(username, password))
    proxy_servers = {
        'http': http_proxy[i]
    }
    print(http_proxy[i])
